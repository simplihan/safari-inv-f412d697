        ...
        <CardContent className="overflow-auto">
          <table className="w-full text-sm">
            <thead className="text-left text-muted-foreground">
              <tr><th className="py-2">User</th><th>Reason</th><th>Out time</th><th>In time</th><th>Duration</th><th>Remarks</th></tr>
            </thead>
            <tbody>
              {filtered.map((r) => (
                <tr key={r.id} className="border-t border-border">
                  <td className="py-2">{profiles[r.user_id]?.full_name ?? <span className="text-muted-foreground">unknown</span>}</td>
                  <td><Badge variant="secondary">{r.reason}</Badge></td>
                  <td className="text-xs whitespace-nowrap">{fmtDateTime(r.out_time)}</td>
